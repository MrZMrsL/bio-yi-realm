  ...
  <div class="character-select-overlay">
    <div class="character-select-panel">
      <div class="select-header">
        <h2 class="select-title">🎭 选择你的专精</h2>
        <p class="select-desc">选择后将决定你的初始属性、技能树和出题范围，一经确认不可更改</p>
      </div>
      ...
      <div class="select-options">
        ...
        >
          ...
          <div
            class="playstyle-tag"
            :style="{ background: opt.themeColor + '22', color: opt.themeColor, borderColor: opt.themeColor + '44' }"
          >
          ...
          <div class="stat-section">
            ...
            <div class="stat-row">
              ...
              <span v-if="opt.initialBonus.critRate > 0" class="stat-item crit"
                >💥 暴击+{{ (opt.initialBonus.critRate * 100).toFixed(0) }}%</span
              >
              <span v-if="opt.initialBonus.dodgeRate > 0" class="stat-item dodge"
                >🌀 闪避+{{ (opt.initialBonus.dodgeRate * 100).toFixed(0) }}%</span
              >
              <span v-if="opt.initialBonus.expBonus > 0" class="stat-item exp"
                >✨ 经验+{{ (opt.initialBonus.expBonus * 100).toFixed(0) }}%</span
              >
...
import { SPECIALIZATIONS } from '../data/specialization.ts'
...
function hasInitialBonus(bonus) {
  return (
    bonus.atk > 0 || bonus.def > 0 || bonus.hp > 0 || bonus.critRate > 0 || bonus.dodgeRate > 0 || bonus.expBonus > 0
  )
}
...
.select-card.selected {
  ...
  box-shadow:
    ...
    inset 0 0 0 1px rgba(212, 168, 83, 0.15);
}
