<template>
  <div class="character-panel">
    ...
    <div class="character-avatar-section">
      ...
      <div class="character-spec" v-if="store.playerSpecialization">
    ...
    <div class="equip-section">
      ...
      <div class="equip-slots">
        <div class="equip-slot" :class="{ filled: store.equipped.weapon }" @click="openEquipList('weapon')">
          ...
          <div class="slot-stat" v-if="store.equipped.weapon">+{{ store.equipped.weapon.atk }} 攻</div>
        </div>
        <div class="equip-slot" :class="{ filled: store.equipped.armor }" @click="openEquipList('armor')">
          ...
          <div class="slot-stat" v-if="store.equipped.armor">+{{ store.equipped.armor.def }} 防</div>
        </div>
        <div class="equip-slot" :class="{ filled: store.equipped.accessory }" @click="openEquipList('accessory')">
          ...
          <div class="slot-stat" v-if="store.equipped.accessory">
    ...
    <div class="stats-section">
      <div class="stats-header">
        ...
        <span class="stats-available">可用: <b>{{ store.statPoints }}</b></span>
      </div>
      <div class="stats-list">
        <div class="stat-row">
          <span class="stat-label">⚔️ 攻击力</span>
          <span class="stat-value">{{ store.atk }} (已分配{{ store.atkPoints }}点)</span>
          <button v-if="store.statPoints > 0" @click="store.allocateStat('atk')" class="btn-allocate">+2</button>
        </div>
        <div class="stat-row">
          <span class="stat-label">🛡️ 防御力</span>
          <span class="stat-value">{{ store.def }} (已分配{{ store.defPoints }}点)</span>
          <button v-if="store.statPoints > 0" @click="store.allocateStat('def')" class="btn-allocate">+2</button>
        </div>
        <div class="stat-row">
          <span class="stat-label">❤️ 生命值</span>
          <span class="stat-value">{{ store.maxHp }} (已分配{{ store.hpPoints }}点)</span>
          <button v-if="store.statPoints > 0" @click="store.allocateStat('hp')" class="btn-allocate">+10</button>
        </div>
      </div>
    ...
    <div v-if="equipPickerOpen" class="equip-picker-overlay" @click.self="equipPickerOpen = false">
      <div class="equip-picker">
        <div class="picker-header">
          ...
          <button @click="equipPickerOpen = false" class="picker-close">×</button>
        </div>
