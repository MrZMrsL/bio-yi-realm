<template>
  <div class="leaderboard-list-section">
    <div class="list-header">
      ...
      <span v-if="totalBattles > 0" class="list-total">共 {{ totalBattles }} 场对决</span>
    </div>
    ...
    <div class="leaderboard-list">
      ...
      <template v-for="(entry, idx) in leaderboard" :key="idx">
        ...
        >
          ...
          <span class="col-rate" :class="getRateClass(entry)"> {{ getWinRate(entry) }}% </span>
        </div>
...
const props = defineProps({
  ...
  expandedRow: { type: Number, default: -1 },
})

const emit = defineEmits(['update:expandedRow'])
...
function toggleExpand(idx) {
  emit('update:expandedRow', props.expandedRow === idx ? -1 : idx)
}
</script>
