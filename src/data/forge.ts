...
import type { ForgeRecipe } from '../types.ts'
...
// 获取可用的配方（按解锁等级过滤）
export function getAvailableRecipes(playerLevel: number): ForgeRecipe[] {
  return FORGE_RECIPES.filter(r => r.unlockLevel <= playerLevel)
}

// 检查是否可以锻造
export function canForge(recipe: ForgeRecipe, inventory: Record<string, number>, gold: number): boolean {
