// specialization.ts - 生化易界专精系统数据
// 每个专精的初始属性、技能树、成长倾向

import type { Specialization, SpecializationBonus } from '../types.ts'
...
// 通过 key 快速查找专精
export function getSpecialization(key: string): Specialization | null {
  return SPECIALIZATIONS.find(s => s.key === key) || null
}

// 获取专精的初始属性加成（计算总值）
export function getInitialBonus(key: string, stat: keyof SpecializationBonus): number {
...
// 获取专精在指定等级已解锁的技能
export function getUnlockedSkills(key: string, level: number) {
  const spec = getSpecialization(key)
  if (!spec) return []
...
// 获取专精在指定等级待解锁的下一个技能
export function getNextSkill(key: string, level: number) {
  const spec = getSpecialization(key)
  if (!spec) return null
...
// 获取专精的整体描述文本（用于预览）
export function getSpecSummary(key: string): (Specialization & { totalSkills: number; skillMilestones: number[] }) | null {
  const spec = getSpecialization(key)
  if (!spec) return null
  return {
    ...spec,
