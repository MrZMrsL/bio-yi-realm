 ...
 * 
 * 专为纯 DOM 渲染的 Vue3 游戏设计，以 Canvas 覆盖层实现高性能粒子特效，
 * 不干扰下方 DOM 元素的点击/交互（pointer-events: none）。
 * 
 * 性能预算：
 *   - 移动端：最多 60 个活跃粒子，超出则丢弃新粒子
 *   - PC 端：最多 120 个活跃粒子
 *   - 粒子生命周期上限 1.2s，到期自动回收
 *   - 使用 requestAnimationFrame，暂停时自动停止循环
 */
...
export function useParticleSystem() {
  ...
  function emitScreenSparks(colors = ['#e74c3c', '#f39c12'], count = 15) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let x, y
      switch (edge) {
        case 0: x = Math.random() * w; y = 0; break          // 上
        case 1: x = w; y = Math.random() * h; break           // 右
        case 2: x = Math.random() * w; y = h; break           // 下
        case 3: x = 0; y = Math.random() * h; break           // 左
      }
